...
aws s3api put-object --bucket {{ cookiecutter.tenant_name }}-{{ cookiecutter.environment }}-dops-tf-state --key sample-database/empty.csv
aws s3api put-object --bucket {{ cookiecutter.tenant_name }}-{{ cookiecutter.environment }}-dops-tf-state --key data-migration/empty.csv